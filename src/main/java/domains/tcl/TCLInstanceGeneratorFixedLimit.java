...
import domains.tcl.primitives.generator.limit.AlternatingLimitGen;
import domains.tcl.simpletcl.SimpleTCLDomain;


public class TCLInstanceGeneratorFixedLimit implements CMDPInstanceGenerator {
	private final int STEPSIZE = 60;
	private final long SEED = 3331L;
	
	public TCLInstanceGeneratorFixedLimit() {
		
	}
	
	public CMDPInstance getInstance(int nAgents, int nDecisions) {
		SimpleTCLDomain mdp = createFixedLimitInstance(nAgents, 80, nDecisions);
	...
	private SimpleTCLDomain createFixedLimitInstance(int numAgents, int numStates, int horizon) {
		InstanceGenerator generator = new FactoredInstanceGenerator(new AnalyticAgentGen(20), new AlternatingLimitGen());
		generator.setSeed(SEED);
		NeighborhoodFactory factory = generator.generateExtendableFactory(numAgents, horizon);
		SimpleTCLDomain domain = new SimpleTCLDomain(factory, 0, 40, numStates, STEPSIZE, horizon*STEPSIZE);
		return domain;
	}
	
	private CMDP[] convertAgentProblems(SimpleTCLDomain mdp) {
